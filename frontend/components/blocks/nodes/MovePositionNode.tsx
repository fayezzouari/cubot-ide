      ...
      <div className="px-4 py-2 text-xs font-medium space-y-1.5">
        {(['x', 'y', 'z'] as const).map((axis) => (
          <div key={axis} className="flex items-center gap-2">
            <span className="w-6 uppercase">{axis}:</span>
            <input
              type="number"
              ...
              onChange={(e) => onDataChange?.(id, { [axis]: parseFloat(e.target.value) || 0 })}
            />
          </div>
        ))}
        <div className="text-[9px] text-white/40 mt-1">Uses IK to move arm</div>
      </div>
